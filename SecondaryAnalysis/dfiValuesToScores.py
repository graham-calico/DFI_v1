...
class Entry:
    ...
    def getDfiValue(self,param):
        ...
        # at the end
        if param=='gait.w':
          if self._inD['hertz']=='N/A': fv = 0.0
          else:
            iv = float(self._inD['hertz'])
            fv = (0.5 - iv) / 0.4
        elif param=='gait.f':
          if self._inD['kpix/sec']=='N/A': fv = 0.0
          else:
            iv = float(self._inD['kpix/sec'])
            fv = (0.06 - iv) / 0.04            
        elif param=='circ.w':
          if self._inD['circ.wh']=='N/A': fv = 0.0
          else:
            iv = float(self._inD['circ.wh'])
            fv = (0.6 - iv) / 0.4
        elif param=='circ.f':
          if self._inD['circ.fl']=='N/A': fv = 0.0
          else:
            iv = float(self._inD['circ.fl'])
            fv = (0.6 - iv) / 0.4
        elif param=='spins':
          if self._inD['spins/day']=='N/A': fv = 0.0
          else:
            iv = float(self._inD['spins/day'])
            fv = (5000 - iv) / 5000.0            
        elif param=='nest':
          if self._inD['bed_mov(pxls)']=='N/A': fv = 0.0
          else:
            iv = float(self._inD['bed_mov(pxls)'])
            fv = (7.5 - iv) / 5.0            
        elif param=='coat.q':
          if self._inD['coat_q(sobel)']=='N/A': fv = 0.0
          else:
          ...
          if self._inD['bwt_dlt(g/day)']=='N/A': fv = 0.0
          else:
            iv = abs(float(self._inD['bwt_dlt(g/day)']))
            fv = iv - 1.0
...
def main():
  ...
  for infname in infileL:
      ...
      for e in entryL:
        ...
        cols.append(sum(dfScoreL) / len(dfScoreL))
